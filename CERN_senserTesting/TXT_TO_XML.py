...
parser = argparse.ArgumentParser(
    description='Covert test results to xml schemas')
parser.add_argument('--f', type=str, help='the .txt filname', required=False)
parser.add_argument(
    '--t', type=str, help='the XML: Tablename you wish to priduce. Available options are: [HGC_CERN_SENSOR_IV,HGC_CERN_SENSOR_IV_SUMRY]', required=False)
parser.add_argument('--user', type=str,
                    help='The name of the user who did this test. Default is Alex.', required=False, default='Alex')
parser.add_argument('--location', type=str,
                    help='The locatino where this test was carried. Default is "FSU"', required=False, default='FSU')
parser.add_argument('--comment', type=str,
                    help='Any comments on this test. Default is the comment from the .txt file', required=False,)
...
FSUDB_OUTPUT_DIR = os.environ['FSUDB_OUTPUT_DIR']  # xml output dir
            ...
            if scratchpad_ID in fullpath:
                ...
                for file in os.listdir(scratchpad_ID_fullpath):
                    if file.endswith('.tex') and "elog" not in file:
                        summary_tex_file_path = os.path.join(
                            scratchpad_ID_fullpath, file)
...
def make_xml_schema_HGC_CERN_SENSOR_IV(filename):
    ...
    # +Run_Name#REMEMBER, SERIAL NUMBER IS SCRATCHPAD ID
    serial_number = IVDICT['Scratchpad_ID']
    if PRESERIES:
        serial_number = serial_number + '_0'

    # Kind_of_part = get_kind_of_part(serial_number, "IV")
    Kind_of_part = '200 um Si Sensor LD Full'  # ???

    xml_table_file = FSUDB_OUTPUT_DIR + Run_Name + \
        '_' + XML_tablename + '_PRESERIES_TEST.xml'
    ...
    with open(xml_table_file, 'w+') as xmlf:
        ...
        xmlf.write('\t<TYPE>\n')
        xmlf.write(
            '\t\t<EXTENSION_TABLE_NAME>HGC_SENSOR_IV</EXTENSION_TABLE_NAME>\n')
        xmlf.write('\t\t<NAME>HGC CERN Sensor IV Test</NAME>\n')
        xmlf.write('\t</TYPE>\n')
        xmlf.write('\t\t<RUN>\n')
        xmlf.write('\t\t\t<RUN_NAME>' + Run_Name + '</RUN_NAME>\n')
        xmlf.write('\t\t\t<RUN_BEGIN_TIMESTAMP>' +
                   IVDICT['Timestamp'].replace("\n", "").rstrip()+'</RUN_BEGIN_TIMESTAMP>\n')
        xmlf.write('\t\t\t<RUN_END_TIMESTAMP>' +
                   IVDICT['Timestamp'].replace("\n", "").rstrip()+'</RUN_END_TIMESTAMP>\n')
        xmlf.write('\t\t\t<INITIATED_BY_USER>' +
                   args.user.rstrip()+'</INITIATED_BY_USER>\n')
        ...
        xmlf.write('\t\t<DATA_SET>\n')
        xmlf.write('\t\t\t<PART>\n')
        xmlf.write('\t\t\t\t<KIND_OF_PART>' +
                   Kind_of_part.rstrip()+'</KIND_OF_PART>\n')
        xmlf.write('\t\t\t\t<SERIAL_NUMBER>' +
                   serial_number.rstrip()+'</SERIAL_NUMBER>\n')
        xmlf.write('\t\t\t</PART>\n')

        for i in range(len(IVDICT['V_list'])):
            xmlf.write('\t\t\t<DATA>\n')
            xmlf.write('\t\t\t\t<VOLTS>' +
                       str(IVDICT['V_list'][i]).rstrip()+'</VOLTS>\n')
            # BELLOW IS CHANNEL CURRENT
            xmlf.write('\t\t\t\t<CURNT_NANOAMP>' +
                       str(IVDICT['Channel_Current_list'][i]).rstrip()+'</CURNT_NANOAMP>\n')
            # BLLOW IS CHANNEL ERROR CURRENT
            xmlf.write('\t\t\t\t<ERR_CURNT_NANOAMP>' +
                       str(IVDICT['Error_Current_list'][i]).rstrip()+'</ERR_CURNT_NANOAMP>\n')
            # BELLOW IS TOT CURRENT
            xmlf.write('\t\t\t\t<TOT_CURNT_NANOAMP>' +
                       str(IVDICT['Tot_Current_list'][i]).rstrip()+'</TOT_CURNT_NANOAMP>\n')
            xmlf.write('\t\t\t\t<ACTUAL_VOLTS>' +
                       str(IVDICT['Act_Volts_list'][i]).rstrip()+'</ACTUAL_VOLTS>\n')
            xmlf.write('\t\t\t\t<TIME_SECS>' +
                       str(IVDICT['Time_list'][i]).rstrip()+'</TIME_SECS>\n')
            xmlf.write('\t\t\t\t<TEMP_DEGC>' +
                       str(IVDICT['Temp_list'][i]).rstrip()+'</TEMP_DEGC>\n')
            xmlf.write('\t\t\t\t<HUMIDITY_PRCNT>' +
                       str(IVDICT['Humidity_list'][i]).rstrip()+'</HUMIDITY_PRCNT>\n')
            # xmlf.write('\t\t\t\t<HUMIDITY_PRCNT>'+str(0.000000E+0)	+'</HUMIDITY_PRCNT>\n')

            xmlf.write('\t\t\t\t<CELL_NR>' +
                       str(IVDICT['Cell_Number_list'][i]).rstrip()+'</CELL_NR>\n')
            xmlf.write('\t\t\t</DATA>\n')
...
def make_xml_schema_HGC_CERN_SENSOR_CV(filename):
    ...
    # Kind_of_part = '200um Si Sensor SD Full'
    # +Run_Name#REMEMBER, SERIAL NUMBER IS SCRATCHPAD ID
    serial_number = CVDICT['Scratchpad_ID']
    if PRESERIES:
        serial_number = serial_number + '_0'
    ...
    xml_table_file = FSUDB_OUTPUT_DIR + Run_Name + \
        '_' + XML_tablename + '_TEST_PRESERIES.xml'
    ...
    with open(xml_table_file, 'w+') as xmlf:
        ...
        xmlf.write('\t\t\t<LOCATION>'+location.rstrip()+'</LOCATION>\n')
        if args.comment:
            xmlf.write('\t\t\t<COMMENT_DESCRIPTION>' +
                       args.comment.rstrip() + '</COMMENT_DESCRIPTION>\n')
        else:
        ...
        for i in range(len(CVDICT['V_list'])):
            xmlf.write('\t\t\t<DATA>\n')
            xmlf.write('\t\t\t\t<VOLTS>' +
                       str(CVDICT['V_list'][i]).rstrip()+'</VOLTS>\n')
            xmlf.write('\t\t\t\t<CPCTNCE_PFRD>' +
                       str(CVDICT['Cs_list'][i]).rstrip()+'</CPCTNCE_PFRD>:\n')
            xmlf.write('\t\t\t\t<ERR_CPCTNC_PFRD>' +
                       str(CVDICT['Error_capacitance_list'][i]).rstrip()+'</ERR_CPCTNC_PFRD>\n')

            xmlf.write('\t\t\t\t<TOT_CURNT_NANOAMP>' +
                       str(CVDICT['Tot_Current_list'][i]).rstrip()+'</TOT_CURNT_NANOAMP>\n')

            xmlf.write('\t\t\t\t<ACTUAL_VOLTS>' +
                       str(CVDICT['Act_Volts_list'][i]).rstrip()+'</ACTUAL_VOLTS>\n')
            xmlf.write('\t\t\t\t<ORG_CPCTNC_PFRD>' +
                       str(CVDICT['Cs_uncorr_list'][i]).rstrip()+'</ORG_CPCTNC_PFRD>\n')
            xmlf.write('\t\t\t\t<TEMP_DEGC>' +
                       str(CVDICT['Temp_list'][i]).rstrip()+'</TEMP_DEGC>\n')
            xmlf.write('\t\t\t\t<HUMIDITY_PRCNT>' +
                       str(CVDICT['Humidity_list'][i]).rstrip()+'</HUMIDITY_PRCNT>\n')
        # xmlf.write('\t\t\t\t<HUMIDITY_PRCNT>'+str(0.000000E+0)	+'<HUMIDITY_PRCNT>\n')
            xmlf.write('\t\t\t\t<IMP_OHM>' +
                       str(CVDICT['Impedence_list'][i]).rstrip()+'</IMP_OHM>\n')
            xmlf.write('\t\t\t\t<PHS_RAD>' +
                       str(CVDICT['Phase_list'][i]).rstrip()+'</PHS_RAD>\n')
            xmlf.write('\t\t\t\t<TIME_SECS>' +
                       str(CVDICT['Time_list'][i]).rstrip()+'</TIME_SECS>\n')
            xmlf.write('\t\t\t\t<CELL_NR>' +
                       str(CVDICT['Cell_Number_list'][i]).rstrip()+'</CELL_NR>\n')
            xmlf.write('\t\t\t</DATA>\n')
